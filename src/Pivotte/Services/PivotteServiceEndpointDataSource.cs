...
{
    public PivotteServiceEndpointDataSource(PivotteServiceDefinition serviceDefinition, string path)
    {
        ...
        foreach (var route in serviceDefinition.Routes)
        {
            var builder = new RouteEndpointBuilder(async context =>
                {
                    await context.RequestServices.GetRequiredService<IPivotteResponseHandler>().HandleResponse(context, route);
                },
